...
*The consolidated ServiceNow coding standards reference for you and your AI agents.*

AI writes ServiceNow code that looks right but breaks ServiceNow rules: slow queries, insecure
patterns, hallucinations. GlideGrail is a consolidated, complete, and well tested set of ServiceNow coding
standards and best practices for anyone building on the platform with AI, written so both
**you and your agents** follow the same rules for **efficiency, security, and consistency**.

Most ServiceNow best-practice material is scattered across community posts, docs, and tribal
knowledge, and written to be read by a human. GlideGrail consolidates years of experience and common rules
...
Grab `SERVICENOW_GLIDEGRAIL_CODING_STANDARDS.md` (clone the repo, or download the raw file)
and wire it into whichever assistant you use.

### Claude & Claude Code
**Claude (claude.ai / desktop / mobile):** attach `SERVICENOW_GLIDEGRAIL_CODING_STANDARDS.md`
...
### ChatGPT / Codex
Paste the file's contents into a **Project's** custom instructions (or a Custom GPT's
knowledge), or attach the file at the start of a coding session and instruct: *"Follow these
ServiceNow standards in all code you write."*

### GitHub Copilot
Add a `.github/copilot-instructions.md` to your repo that says to follow the standards file,
and keep the file in the repo so Copilot's workspace context can read it.

### Gemini
Attach the file (or paste it) into a **Gem's** instructions, or include it at the top of your
prompt/context for the session.

### Cursor
Add it as a project rule: drop the file in `.cursor/rules/` (or reference it from
`.cursorrules`) so it's loaded into every request automatically.
...
`SERVICENOW_GLIDEGRAIL_CODING_STANDARDS.md` covers, as enforceable conventions:
